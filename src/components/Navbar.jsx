    ...
    <nav className="bg-white dark:bg-slate-800 shadow-sm md:sticky top-0 z-50">
      <div className="container flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          >
            {dark ? <FaSun className="text-yellow-400" /> : <FaMoon className="text-slate-600" />}
            {dark ? 'Light' : 'Dark'}
          </button>
        </div>
        ...
        <div className="md:hidden flex items-center gap-3">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700"
          >
            {isOpen ? <FaTimes className="text-xl text-slate-700 dark:text-slate-200" /> : <FaBars className="text-xl text-slate-700 dark:text-slate-200" />}
          </button>
        </div>
      </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white dark:bg-slate-800 shadow-lg z-50">
          <div className="flex flex-col p-4 gap-4">
            <NavLink to="/" onClick={() => setIsOpen(false)} className={({isActive}) => isActive ? 'text-brand font-medium' : 'text-slate-600 dark:text-slate-300'}>Home</NavLink>
            <NavLink to="/mission" onClick={() => setIsOpen(false)} className={({isActive}) => isActive ? 'text-brand font-medium' : 'text-slate-600 dark:text-slate-300'}>Mission</NavLink>
            <NavLink to="/programs" onClick={() => setIsOpen(false)} className={({isActive}) => isActive ? 'text-brand font-medium' : 'text-slate-600 dark:text-slate-300'}>Programs</NavLink>
            <NavLink to="/contact" onClick={() => setIsOpen(false)} className={({isActive}) => isActive ? 'text-brand font-medium' : 'text-slate-600 dark:text-slate-300'}>Contact</NavLink>
            <Link to="/contact" onClick={() => setIsOpen(false)} className="text-sm px-3 py-2 bg-brand text-white rounded-lg text-center">Get Involved</Link>
          </div>
        </div>
      )}
    </nav>
